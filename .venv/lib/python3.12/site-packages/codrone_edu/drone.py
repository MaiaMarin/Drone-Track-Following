...
class Drone:
    ...
    def __init__(self, flagCheckBackground=True, flagShowErrorMessage=False, flagShowLogMessage=False,
                 flagShowTransferData=False, flagShowReceiveData=False, swarm=False, color_data_dir=None):

        self._serialport = None
        if sys.platform == 'emscripten':
            self._serialport = serialPortModule.SerialPortManager
        ...
        self.setEventHandler(DataType.Ack, self.update_ack_data)
        ...
        if sys.platform != 'emscripten':
            ...
            self.parent_dir = color_data_dir
        else:
            self.parent_dir = '/home/web_user/data/'
    ...
    async def _trigger_callback(self, new_data, name):
        trim_data = await self.get_trim()
        if name == "roll" and self.roll_pitch_callback != None:
            self.roll_pitch_callback(new_data, trim_data[1])
        if name == "pitch" and self.roll_pitch_callback != None:
            self.roll_pitch_callback(trim_data[0], new_data)

    async def _set_roll_trim(self, roll):
        await self._trigger_callback(roll, 'roll')

    async def _set_pitch_trim(self, pitch):
        await self._trigger_callback(pitch, 'pitch')
    ...
    def update_ack_data(self, drone_type):
        """
        systemTime	Int64	 8 Byte -   System time
        dataType	DataType 1 Byte -   Received data type
        crc16	    Int16	 2 Byte	-	CRC16 value of received header and data
        """

        self.ack_data[0] = time.time() - self.timeStartProgram
        self.ack_data[1] = drone_type.systemTime
        self.ack_data[2] = drone_type.dataType
        self.ack_data[3] = drone_type.crc16

    def get_ack_data(self, delay=0.1):
        """
        Will get and print the current Ack data
        :param delay:
        :return:
        """
        if sys.platform != 'emscripten' and not self._swarm:
            return self._get_ack_data_desktop(delay)
        else:
            return asyncio.create_task(self._get_ack_data_emscripten(delay))

    def _get_ack_data_desktop(self, delay=0.1):
        self.sendRequest(DeviceType.Drone, DataType.Ack)
        time.sleep(delay)
        return self.ack_data

    async def _get_ack_data_emscripten(self, delay=0.1):
        await self.sendRequest(DeviceType.Drone, DataType.Ack)
        await asyncio.sleep(delay)
        return self.ack_data
    ...
    async def _get_state_data_emscripten(self, delay=0.01):
        await self.sendRequest(DeviceType.Drone, DataType.State)
        await asyncio.sleep(delay)
        self._system = self.state_data[1]
        self._battery = self.state_data[6]
        self.state_data[1] = ModeSystem.Error
        self.state_data[6] = 0
        return self.state_data
    ...
    async def _get_battery_emscripten(self):
        await self.get_state_data(delay=0.02)
    ...
    def get_system_state(self):
        if sys.platform != 'emscripten' and not self._swarm:
            return self._get_system_state_desktop()
        else:
            return asyncio.create_task(self._get_system_state_emscripten())

    def _get_system_state_desktop(self):
        return self.get_state_data()[1]

    async def _get_system_state_emscripten(self):
        await self.get_state_data()
        system = self._system
        self._system = ModeSystem.Error
        return system
    ...
    def go(self, direction, power=50, duration=1):
        ...
        :param power: int the power at which the drone flies. Takes a value from 0 to 100. Defaults to 50 if not defined.
        :param duration: int duration of the flight motion in seconds. If undefined, defaults to 1 second.
        """
        if sys.platform != 'emscripten' and not self._swarm:
            return self._go_desktop(direction, power, duration)
        else:
    ...
    async def _receiving_emscripten(self):
        while self._flagThreadRun:
            value = await self._serialport.read()
    ...
    async def _isOpen_emscripten(self):
        if self._serialport is not None and self._serialport.port is not None:
            try:
                await self._serialport.port.getSignals()
                return True
            except:
                return False
        else:
    ...
    async def _transfer_emscripten(self, header, data):
        ...
        self._serialport.write(j(dataArray))
    ...
    def _land_desktop(self):
        ...
        #########
        # When taking off multiple times in one run, there exists 3 coordinate systems in the entire run: G_new, G_origin, L
        # G_origin is the coordinate system of the first takeoff (global).
        # G_new is the coordinate system of the most recent takeoff (global), which can have a different heading than G_origin
        # L is the internal coordinate system of the drone itself (local), which can have a different heading than G_new and G_origin.
        #   - L's initial heading is equal to G_new's heading
        #########

        # calculate how much previous_land values need to shift, but the values are in G_new frame
        landing_dx_prime = self.get_position_data()[1]
        landing_dy_prime = self.get_position_data()[2]
        landing_dheading_prime = self.get_angle_z()

        # transform landing_dx_prime and landing_dy_prime to be in G_origin frame using inverse rotation matrix
        prev_land_z_angle = (pi / 180) * self.previous_land[2]
        landing_dx = landing_dx_prime * cos(prev_land_z_angle) - landing_dy_prime * sin(prev_land_z_angle)
        landing_dy = landing_dx_prime * sin(prev_land_z_angle) + landing_dy_prime * cos(prev_land_z_angle)

        # update previous_land values
        self.previous_land[0] = self.previous_land[0] + landing_dx
        self.previous_land[1] = self.previous_land[1] + landing_dy
        # constrain previous_land[2] to be between -180 to 180
        self.previous_land[2] = ((self.previous_land[2] + landing_dheading_prime + 180) % 360) - 180
    ...
    async def _land_emscripten(self):
        ...
        # calculate how much previous_land values need to shift, but the values are in G_new frame
        landing_dx_prime = (await self.get_position_data())[1]
        landing_dy_prime = (await self.get_position_data())[2]
        landing_dheading_prime = await self.get_angle_z()
    ...
    def set_roll(self, power):
        ...
        :return: None
        """
        self._control.roll = max(-100, min(100, int(power)))
    ...
    def set_pitch(self, power):
        ...
        :return: None
        """
        self._control.pitch = max(-100, min(100, int(power)))
    ...
    def set_yaw(self, power):
        ...
        :return: None
        """
        self._control.yaw = max(-100, min(100, int(power)))
    ...
    def set_throttle(self, power):
        ...
        :return: None
        """
        self._control.throttle = max(-100, min(100, int(power)))
    ...
    def _move_distance_desktop(self, positionX, positionY, positionZ, velocity):
        ...
        wait = (distance / velocity) + 1.25

        time.sleep(0.1)

        while self.get_movement_state() != ModeMovement.Moving:
    ...
    async def _move_distance_emscripten(self, positionX, positionY, positionZ, velocity):
        ...
        while (await self.get_movement_state()) != ModeMovement.Moving:
    ...
    def _move_forward_desktop(self, distance, units="cm", speed=0.5):
        ...
        time.sleep(0.1) # giving it enough time to transition to hover state if it did skip

        while self.get_movement_state() != ModeMovement.Moving:
            self.sendControlPosition(positionX=distance_meters, positionY=0, positionZ=0, velocity=speed,
                                     heading=0,
                                     rotationalVelocity=0)
            time.sleep(0.1)
    ...
    async def _move_forward_emscripten(self, distance, units="cm", speed=0.5):
        ...
        while (await self.get_movement_state()) != ModeMovement.Moving:
            await self.sendControlPosition(positionX=distance_meters, positionY=0, positionZ=0, velocity=speed,
                                     heading=0,
                                     rotationalVelocity=0)
    ...
    def _move_left_desktop(self, distance, units="cm", speed=0.5):
        ...
        while self.get_movement_state() != ModeMovement.Moving:
            self.sendControlPosition(positionX=0, positionY=distance_meters, positionZ=0, velocity=speed,
                                     heading=0,
                                     rotationalVelocity=0)
            time.sleep(0.1)
    ...
    async def _move_left_emscripten(self, distance, units="cm", speed=0.5):
        ...
        while (await self.get_movement_state()) != ModeMovement.Moving:
            await self.sendControlPosition(positionX=0, positionY=distance_meters, positionZ=0, velocity=speed,
                                     heading=0,
                                     rotationalVelocity=0)
            await asyncio.sleep(0.10)
    ...
    def _send_absolute_position_desktop(self, positionX, positionY, positionZ, velocity, heading, rotationalVelocity):
        ...
        # store current position and z_angle
        pos_data = None
        z_angle = None # angle offset between G_new and L
        if self.information_data[1] == ModelNumber.Drone_12_Drone_P1:
            for _ in range(3):
        ...
        z_angle_land = self.previous_land[2] # angle offset between G_origin and G_new

        # constrain z-angle value to be between -180 to 180
        abs_z_angle = (((z_angle_land + z_angle) + 180) % 360) - 180 # angle offset between G_origin and L, z_angle_land + z_angle

        abs_z_angle_rad = (pi / 180) * abs_z_angle
        ...
        # G_origin, G_new, and L are essentially the same if the drone never rotates or stays at 0 heading
        # G_origin and G_new are essentially the same if the drone lands at 0 heading
        # G_new and L are essentially the same if the drone never rotates or stays at 0 heading

        # pos_data and z_angle variable are data from G_new's coordinate system
        #   - z_angle is the angle offset between G_new and L
        # positionX, positionY, positionZ, pos_x_land, pos_y_land, z_angle_land, and abs_z_angle variables are data from G_origin's coordinate system
        #   - z_angle_land is the angle offset between G_origin and G_new
        #   - abs_z_angle is the angle offset between G_origin and L, essentially it is "z_angle + z_angle_land"

        # ControlPosition()'s inputs are x,y,z, and heading displacements based off of the drone's current orientation,
        # AKA the L coordinate system.

        # The position variables mentioned above must be converted into values, so that it fits with the L coordinate system.
        #   - This is done by applying rotation matrix on x, y values based off of the angle offset between coordinate systems.

        # Since pos_data is G_new data, pos_data's x and y data needs to "rotate" by z_angle
        # Since positionX, positionY, pos_x_land, and pos_y_land is G_origin data, they need to "rotate" by abs_z_angle

        # Now, we can calculate the displacements for x and y in the L coordinate system.
        #########

        # apply rotation matrix to position values with their respective offsets
        # from G_new to L
        pos_x_prime = pos_data[1] * cos(z_angle_rad) + pos_data[2] * sin(z_angle_rad)
        pos_y_prime = -pos_data[1] * sin(z_angle_rad) + pos_data[2] * cos(z_angle_rad)

        # from G_origin to L
        positionX_prime = positionX * cos(abs_z_angle_rad) + positionY * sin(abs_z_angle_rad)
        positionY_prime = -positionX * sin(abs_z_angle_rad) + positionY * cos(abs_z_angle_rad)

        # from G_origin to L
        pos_x_land_prime = self.previous_land[0] * cos(abs_z_angle_rad) + self.previous_land[1] * sin(abs_z_angle_rad)
        pos_y_land_prime = -self.previous_land[0] * sin(abs_z_angle_rad) + self.previous_land[1] * cos(abs_z_angle_rad)

        # compute deltas in L coordinate system, final_pos - (pos_land + initial_pos)
        dx_prime = positionX_prime - (pos_x_land_prime + pos_x_prime)
        dy_prime = positionY_prime - (pos_y_land_prime + pos_y_prime)
        dz_prime = positionZ - pos_data[3]
        # computes shortest possible delta needed to travel from "abs_z_angle" to "heading"
        dheading = ((int(heading) - abs_z_angle + 180) % 360) - 180

        # calculate the deltas in position needed to fly in those axes
        data.positionX = dx_prime
        data.positionY = dy_prime
        data.positionZ = dz_prime
        data.velocity = float(velocity)
        data.heading = dheading
        data.rotationalVelocity = rotationalVelocity
    ...
    async def _send_absolute_position_emscripten(self, positionX, positionY, positionZ, velocity, heading, rotationalVelocity):
        ...
        # pos_data and z_angle variable are data from G_new's coordinate system
        #   - z_angle is the angle offset between G_new and L
        # positionX, positionY, positionZ, pos_x_land, pos_y_land, z_angle_land, and abs_z_angle variables are data from
        #   - z_angle_land is the angle offset between G_origin and G_new
        #   - abs_z_angle is the angle offset between G_origin and L, essentially it is "z_angle + z_angle_land"
        # G_origin's coordinate system.
    ...
    def reset_previous_land(self):
        self.previous_land = [0, 0, 0]
    ...
    def set_drone_LED_mode(self, r, g, b, mode, speed):
        """
        Changes the drone LED to a specified color and mode using RGB values.

        :param r: integer from 0-255
        :param g: integer from 0-255
        :param b: integer from 0-255
        :param mode: string representing a mode (e.g. "dimming")
        :param speed: integer from 1-10
        :return: None
        """
        if sys.platform != 'emscripten' and not self._swarm:
            return self._set_drone_LED_mode_desktop(r, g, b, mode, speed)
        else:
            return asyncio.create_task(self._set_drone_LED_mode_emscripten(r, g, b, mode, speed))

    def _set_drone_LED_mode_desktop(self, r, g, b, mode, speed):
        ...
        if mode == "dimming" or mode == DroneLEDMode.Dimming:
            interval = (11-speed) * (50 // 10) # interval ranges [10,50]
            self.sendLightDefaultColor(LightModeDrone.BodyDimming, interval=interval, r=r, g=g, b=b)
        elif mode == "fade_in"  or mode == DroneLEDMode.Fade_In:
            interval = (11-speed) * (127 // 10) # interval ranges [12,127]
            self.sendLightDefaultColor(LightModeDrone.BodySunrise, interval=interval, r=r, g=g, b=b)
        elif mode == "fade_out"  or mode == DroneLEDMode.Fade_Out:
            interval = (11-speed) * (127 // 10) # interval ranges [12,127]
            self.sendLightDefaultColor(LightModeDrone.BodySunset, interval=interval, r=r, g=g, b=b)
        elif mode == "blink"  or mode == DroneLEDMode.Blink:
            interval = (11-speed) * (1000 // 10) # interval ranges [100,1000]
            self.sendLightDefaultColor(LightModeDrone.BodyFlicker, interval=interval, r=r, g=g, b=b)
        elif mode == "double_blink" or mode == DroneLEDMode.Double_Blink:
            interval = (11-speed) * (600 // 10) # interval ranges [60,600]
            self.sendLightDefaultColor(LightModeDrone.BodyFlickerDouble, interval=interval, r=r, g=g, b=b)
        elif mode == "rainbow" or mode == DroneLEDMode.Rainbow:
            interval = (11-speed) * (75 // 10) # interval ranges [7,75]
            self.sendLightDefaultColor(LightModeDrone.BodyRainbow, interval=interval, r=r, g=g, b=b)
        else:
            print(Fore.YELLOW + "Warning: Invalid light mode" + Style.RESET_ALL)

        time.sleep(0.005)

    async def _set_drone_LED_mode_emscripten(self, r, g, b, mode, speed):
        # speed ranges [1, 10]
        if speed > 10:
            speed = 10
        if speed <= 0:
        ...
        if mode == "dimming" or mode == DroneLEDMode.Dimming:
            interval = (11 - speed) * (50 // 10)  # interval ranges [10,50]
            await self.sendLightDefaultColor(LightModeDrone.BodyDimming, interval=interval, r=r, g=g, b=b)
        elif mode == "fade_in"  or mode == DroneLEDMode.Fade_In:
            interval = (11 - speed) * (127 // 10)  # interval ranges [12,127]
            await self.sendLightDefaultColor(LightModeDrone.BodySunrise, interval=interval, r=r, g=g, b=b)
        elif mode == "fade_out" or mode == DroneLEDMode.Fade_Out:
            interval = (11 - speed) * (127 // 10)  # interval ranges [12,127]
            await self.sendLightDefaultColor(LightModeDrone.BodySunset, interval=interval, r=r, g=g, b=b)
        elif mode == "blink" or mode == DroneLEDMode.Blink:
            interval = (11 - speed) * (1000 // 10)  # interval ranges [100,1000]
            await self.sendLightDefaultColor(LightModeDrone.BodyFlicker, interval=interval, r=r, g=g, b=b)
        elif mode == "double_blink" or mode == DroneLEDMode.Double_Blink:
            interval = (11 - speed) * (600 // 10)  # interval ranges [60,600]
            await self.sendLightDefaultColor(LightModeDrone.BodyFlickerDouble, interval=interval, r=r, g=g, b=b)
        elif mode == "rainbow" or mode == DroneLEDMode.Rainbow:
            interval = (11 - speed) * (75 // 10)  # interval ranges [7,75]
            await self.sendLightDefaultColor(LightModeDrone.BodyRainbow, interval=interval, r=r, g=g, b=b)
        else:
            print("Warning: Invalid light mode", color="warning")
    ...
    def set_controller_LED_mode(self, r, g, b, mode, speed):
        """
        Changes the controller LED to a specified color and mode using RGB values.
        ...
        """
        if sys.platform != 'emscripten' and not self._swarm:
            return self._set_controller_LED_mode_desktop(r, g, b, mode, speed)
        else:
            return asyncio.create_task(self._set_controller_LED_mode_emscripten(r, g, b, mode, speed))

    def _set_controller_LED_mode_desktop(self, r, g, b, mode, speed):
        ...
        if mode == "dimming" or mode == ControllerLEDMode.Dimming:
            interval = (11 - speed) * (50 // 10)  # interval ranges [10,50]
            self.sendLightDefaultColor(LightModeController.BodyDimming, interval=interval, r=r, g=g, b=b)
        elif mode == "fade_in" or mode == ControllerLEDMode.Fade_In:
            interval = (11 - speed) * (127 // 10)  # interval ranges [12,127]
            self.sendLightDefaultColor(LightModeController.BodySunrise, interval=interval, r=r, g=g, b=b)
        elif mode == "fade_out" or mode == ControllerLEDMode.Fade_Out:
            interval = (11 - speed) * (127 // 10)  # interval ranges [12,127]
            self.sendLightDefaultColor(LightModeController.BodySunset, interval=interval, r=r, g=g, b=b)
        elif mode == "blink" or mode == ControllerLEDMode.Blink:
            interval = (11 - speed) * (1000 // 10)  # interval ranges [100,1000]
            self.sendLightDefaultColor(LightModeController.BodyFlicker, interval=interval, r=r, g=g, b=b)
        elif mode == "double_blink" or mode == ControllerLEDMode.Double_Blink:
            interval = (11 - speed) * (600 // 10)  # interval ranges [60,600]
            self.sendLightDefaultColor(LightModeController.BodyFlickerDouble, interval=interval, r=r, g=g, b=b)
        elif mode == "rainbow" or mode == ControllerLEDMode.Rainbow:
            interval = (11 - speed) * (75 // 10)  # interval ranges [7,75]
            self.sendLightDefaultColor(LightModeController.BodyRainbow, interval=interval, r=r, g=g, b=b)
        else:
            print(Fore.YELLOW + "Warning: Invalid light mode" + Style.RESET_ALL)

        time.sleep(0.005)

    async def _set_controller_LED_mode_emscripten(self, r, g, b, mode, speed):
        # speed ranges [1, 10]
        if speed > 10:
            speed = 10
        if speed <= 0:
        ...
        if mode == "dimming" or mode == ControllerLEDMode.Dimming:
            interval = (11 - speed) * (50 // 10)  # interval ranges [10,50]
            await self.sendLightDefaultColor(LightModeController.BodyDimming, interval=interval, r=r, g=g, b=b)
        elif mode == "fade_in" or mode == ControllerLEDMode.Fade_In:
            interval = (11 - speed) * (127 // 10)  # interval ranges [12,127]
            await self.sendLightDefaultColor(LightModeController.BodySunrise, interval=interval, r=r, g=g, b=b)
        elif mode == "fade_out" or mode == ControllerLEDMode.Fade_Out:
            interval = (11 - speed) * (127 // 10)  # interval ranges [12,127]
            await self.sendLightDefaultColor(LightModeController.BodySunset, interval=interval, r=r, g=g, b=b)
        elif mode == "blink" or mode == ControllerLEDMode.Blink:
            interval = (11 - speed) * (1000 // 10)  # interval ranges [100,1000]
            await self.sendLightDefaultColor(LightModeController.BodyFlicker, interval=interval, r=r, g=g, b=b)
        elif mode == "double_blink" or mode == ControllerLEDMode.Double_Blink:
            interval = (11 - speed) * (600 // 10)  # interval ranges [60,600]
            await self.sendLightDefaultColor(LightModeController.BodyFlickerDouble, interval=interval, r=r, g=g, b=b)
        elif mode == "rainbow" or mode == ControllerLEDMode.Rainbow:
            interval = (11 - speed) * (75 // 10)  # interval ranges [7,75]
            await self.sendLightDefaultColor(LightModeController.BodyRainbow, interval=interval, r=r, g=g, b=b)
        else:
            print("Warning: Invalid light mode", color="warning")
    ...
    def set_trim(self, roll, pitch):
        ...
        if sys.platform != 'emscripten' and not self._swarm:
            ...
            return asyncio.create_task(self._set_trim_swarm(roll, pitch))
        else:
    ...
    async def _set_trim_swarm(self, roll, pitch):
        roll = int(roll)
        pitch = int(pitch)
        for _ in range(3):
    ...
    async def _set_trim_emscripten(self, roll, pitch):
        ...
        await self._set_roll_trim(roll)
        await self._set_pitch_trim(pitch)
    ...
    def reset_trim(self):
        ...
        if sys.platform != 'emscripten' and not self._swarm:
            ...
            return asyncio.create_task(self._reset_trim_swarm())
        else:
    ...
    async def _reset_trim_swarm(self):
        for _ in range(3):
    ...
    async def _reset_trim_emscripten(self):
        ...
        await self._set_roll_trim(0)
        await self._set_pitch_trim(0)
    ...
    def detect_colors(self, color_data):
        try:
            ...
            predict_colors_error = "A dataset has not been loaded."
            if sys.platform != 'emscripten':
    ...
    def load_color_data(self, dataset=None, show_graph=False):
        ...
        for filename in os.listdir(path):
            ...
            prev_sample_size = sample_size
            all_labels.append(filename[:-4]) # append the label to the list
    ...
    def print_num_data(self, label, dataset):
        folder = dataset
        path = os.path.join(self.parent_dir, folder)
